...
When writing backend code (database schema, RLS policies, API endpoints, business logic in `fenzit-be`), think **as a backend engineer**, not as a frontend engineer responding to frontend prompts. Frontend requirements can lead to client-side assumptions that break security, data integrity, concurrency safety, and multi-tenant isolation. Never trust the client.Ensure fenzit-be backend quality by maintaining strict foreign key and check constraints, enforcing explicit deny-by-default RLS security, preventing race conditions via idempotency headers and atomic operations, establishing consistent API contracts and error logging without leaking secrets, and briefing AI agents with backend-first constraints, API definitions, and post-implementation code review checklists.
When writing frontend code (screens, components, hooks, state management), think **as a frontend/UX/React Native engineer**, not as a backend engineer trying to mirror API responses. Backend capabilities don't translate directly to frontend performance, UX, or accessibility. Drive fenzo-app frontend development with a UX-first mindset by strictly enforcing design system tokens and atomic components, applying tiered state management, optimizing performance and resilience with list virtualization, client-side validation, and explicit loading/error/empty states, adhering to accessibility standards and briefing AI dev agents using design-focused prompts, comprehensive checklists, and pre-merge code reviews.
