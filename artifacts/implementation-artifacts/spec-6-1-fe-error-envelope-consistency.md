# Story 6.1: FE Error Envelope Consistency

Status: ready-for-dev

> **Moved 2026-09-11** from the fenzo-app repo-local sprint (was `5-4-error-envelope-consistency`,
> `workspace/core/frontend/fenzo-app/_bmad-output/implementation-artifacts/5-4-error-envelope-consistency.md`).
> The meta sprint is now the only active sprint. All code changes below land in **fenzo-app**;
> commit in that repo.
...
**Implementation-time note (2026-09-11):** this fact is now stale — `flattenErrorMessage` was already
centralized in `toApiError` during the 5-2 review (2026-09-04). Remaining gaps: `filter(Boolean)`
on the join, aligning 5xx default copy to `errorCopy.SERVER`, `errorCopy` module, shim removal,
audit, CLAUDE.md section.
