...
## Deferred from: code review of spec-4-5 (2026-09-11)
- CR4.5-D1 — **RPC-write → re-fetch race window** — a concurrent advance/confirm mutating the job between the write RPC and `refetchWithEmbeds` can make the response report a newer step/status than the operation performed. Benign for the client (sees fresher state); a snapshot fix needs the RPC to return the joined row — an RPC/migration change, Ask-First in this spec. [Source: src/jobs/jobs.service.ts refetchWithEmbeds]
- CR4.5-D2 — **FK embeds never executed against real PostgREST** — every suite (unit + e2e + integration) mocks `SupabaseClientFactory`, so an invalid embed (bad relationship name, dropped FK) would 400 every job read in production while all suites stay green. Story 4.2 has a real-DB precedent in `rls-isolation.integration.spec.ts`; extending it is out of this story's boundaries (spec keeps that file untouched). Fold into the CI-DB enablement bucket (CR3.7-D1). [Source: src/jobs/jobs.service.ts JOB_COLUMNS]

- source_spec: `artifacts/implementation-artifacts/spec-5-2-dynamic-technician-workflow-from-template-steps.md`
  summary: Signature-capture threading (`useSignatureSave`, `SignatureScreen`, `Signature` route param, `TechJobDetailScreen`'s capture/recapture navigation) stays hardcoded to the literal step key `'signature_captured'` and the old `STEP_ORDER` for its 422 reconcile — not cut over to the job's own `workflowTemplate.steps` in this story.
  evidence: Split off Story 5.2's spec to stay under the token ceiling (user chose [S] Split at the 2026-09-11 checkpoint over [K] Keep full spec ~1800-3000 tokens). Story 5.2 narrows to the stepper/action-bar/advance-hook cutover (`stepperModel`, `WorkflowStepper`, `workflowActionBarModel`, `useWorkflowAdvance`'s reconcile generalization, `jobs.ts` type widening, and the `TechJobDetailContent` signature-card visibility fix, which only reads template data and needs no signature-hook change). Accepted rough edge: `useWorkflowAdvance`'s signature reroute becomes template-driven for DETECTION (checks the target step's `requiresSignature` instead of the literal key) but still calls `onCaptureSignature()` with no step-key argument — so for any skill template whose signature step key is NOT `'signature_captured'`, the Signature screen will still POST the wrong literal key and 422 until this follow-up lands. Follow-up must: thread `stepKey` through `onCaptureSignature`, the `Signature` route param (`navigation/types.ts`), `SignatureScreen.tsx`, and `useSignatureSave.ts` (POST + reconcile against the job's own template instead of `STEP_ORDER.indexOf('signature_captured')`).

- source_spec: `artifacts/implementation-artifacts/7-10-frontend-technician-distance-from-job-site.md`
  summary: Core `Clipboard` deprecated in RN (moved to `@react-native-clipboard/clipboard`) — `JobDetailScreen.tsx` copy affordances use the core module deliberately (no new dependency on RN 0.86; it still works). Revisit when the next RN upgrade lands or when the clipboard package is added for another feature; at that point swap the import and drop the core usage.

- source_spec: `artifacts/implementation-artifacts/7-10-frontend-technician-distance-from-job-site.md`
  summary: Unknown `step_*` event keys render their raw string (e.g. `step_on_my_way`) in the timeline and now the header's latest-event badge — the fallback mirrors `eventLabel`'s never-crash rule, but the wording shown to users for unknown steps is a product decision (e.g. a friendlier generic label) for a later pass.
...
- [x] **Database functions callable directly with the public key** (fenzit-be) — RESOLVED by story 14-1 (2026-09-25): migration `fenzit-be/supabase/migrations/20260925000001_revoke_public_execute_on_db_functions.sql` revoked EXECUTE on all 10 public-schema functions from PUBLIC/anon/authenticated (service_role + owner grants kept — every app call site routes through `createAdmin()`); verified live (`pg_proc.proacl` = `{postgres=X,service_role=X}` per function) and pinned by direct-RPC probes (authenticated + anon keys → 42501) in `test/integration/rls-isolation.integration.spec.ts`. Original finding: Postgres grants EXECUTE to PUBLIC by default and no migration revoked it on the job/workflow RPCs (e.g. `advance_workflow_step`), which trust caller-supplied tenant/actor IDs. The Supabase publishable key ships in fenzo-app (`src/config/index.ts`, used only by `src/services/supabaseRealtime.ts` for live notifications — that part is correct and stays). Details: `artifacts/planning-artifacts/prds/prd-Fenzo-attendance-2026-09-25/review-backend.md`, `review-database.md`.
- [x] **`users_update_own` policy has no column limit** (fenzit-be) — RESOLVED by story 14-1 (2026-09-25): migration `fenzit-be/supabase/migrations/20260925000002_column_limit_users_update_own.sql` revoked whole-table UPDATE from anon/authenticated and granted `UPDATE (name)` to authenticated only (anon loses UPDATE entirely; service_role/postgres untouched). PostgreSQL 17 has no column-list policies (`FOR UPDATE (name)` = syntax error 42601, probed live), so the column limit is carried entirely by the grant-level check — which Postgres evaluates before RLS, so a combined `SET name, role` update is denied (42501) even on a zero-row match. Verified live and pinned by combined/name-only probes in `test/integration/rls-isolation.integration.spec.ts`.
- **Risk note (historical):** today only owners get a realtime token. Attendance FR-27 gives one to every technician, which widens exposure. Revisit these two items before FR-27 ships. User chose to defer (2026-09-25); both were then moved into scope and shipped (see above) — the AD-18 fix is implemented (merge/deploy pending commit).
...
## Deferred from: BMAD build review of spec-14-1-backend-security-prerequisite-job-rpc-and-users-update-own.md

- source_spec: `artifacts/implementation-artifacts/spec-14-1-backend-security-prerequisite-job-rpc-and-users-update-own.md`
  summary: The story-14-1 security probes (and every other real-DB integration probe in test/integration/) run in no automated path — jest's rootDir is src/ so `bun run test` never loads them, and `bun run test:e2e` skips them without real Supabase credentials; CI has no test step or DB.
  evidence: Verification-gap reviewer traced package.json:91-93 (jest rootDir "src"), test/jest-e2e.json (matches .integration.spec.ts, gated on IS_REAL_DB), and .github/workflows/render-status.yml (deploy-status poll only). The security ACL state established by migrations 20260925000001-3 is therefore machine-verified only when someone manually runs `bun run test:e2e` with real credentials. Same CI-has-no-DB infra gap already noted under J1/E2/W2/AR-20 — fold into the CI-DB enablement work.
- **Migrate the six legacy job/workflow RPCs to backend application code** (fenzit-be) — user decision 2026-09-21 (commit `87bc201` in fenzit-meta): keep stored procedures to a minimum; business logic lives in NestJS app code. The decision landed after these RPCs were built, and story 14-1 only locked them down (EXECUTE revokes), not migrated them. `create_job_with_log`, `advance_workflow_step`, `update_job_with_log`, `confirm_attachment`, `increment_job_counter`, `setup_tenant_for_owner` still carry multi-table atomic writes (state change + activity log) as SECURITY DEFINER functions. Migration plan per table-family: express the write as a guarded UPDATE from the NestJS service where possible (single-table transitions — copy the 12-3 report-worker claim pattern); for genuinely multi-table atomic cases (state change + activity_logs insert in one statement), supabase-js/PostgREST offers no transaction — options are (a) restructure so the activity-log write is a DB trigger on the state change, or (b) accept an RPC exception per instance with a recorded justification. Rule now codified as `fenzit-be/project-context.md` rule #6 — no NEW DB functions. Not urgent: the RPCs are service-role-only since 14-1 and their behaviour is pinned by tests; do this as its own refactor story before extending job-flow logic.

## Deferred from: code review of spec-14-1-backend-security-prerequisite-job-rpc-and-users-update-own (2026-09-25)
